...
AcomOfferDesk - веб-платформа для работы с заявками и офферами между сотрудниками и контрагентами, с интеграцией Keycloak и уведомлений.

## Быстрые ссылки

- Инструкция по автопривязке пользователей к Keycloak: [docs/keycloak-autolink.md](docs/keycloak-autolink.md)
- Правила входа по внешним ссылкам (email): [docs/login-links.md](docs/login-links.md)
...
- Пустой `flyway/sql` на сервере (миграции не накатываются → нет `user_auth_accounts`, 500 на OAuth callback): раздел «Рассинхрон: пустой flyway/sql» в [order-database-vps.md](docs/order-database-vps.md)
...
- Telegram (legacy, disabled by default): aiogram
...
### 1) Контрагент регистрируется самостоятельно (через email invite)
...
- Прямой вход из Telegram отключен (`legacy /auth/tg/login` только перенаправляет на обычный вход, `legacy /api/v1/auth/tg/exchange` запрещен).
